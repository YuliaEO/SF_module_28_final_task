...
from selenium.webdriver.common.action_chainsaction_chains import ActionChains
...
def test_correct_redirect_to_register(): # TK-001 Проверка перехода по ссылке "Зарегистрироваться"
...
def test_registration_phoneNumber(): # TK-002 Проверка регистрации по номеру телефона - страница Регистрации
    ...
    actionChain.send_keys('Latinica2').perform()
    time.sleep(2) # Для наглядности - в реальности тестирования закомментировать
    ...
    time.sleep(5) # Для наглядности - в реальности тестирования закомментировать
    driver.find_element(By.NAME, 'register').click()
    time.sleep(3) # Для наглядности - в реальности тестирования закомментировать
...
def test_registration_email(): # TK-003 Проверка регистрация по email  - страница Регистрации
    driver, wait = get_driver()
    actionChain = ActionChains(driver)
...
def test_register_firstName_and_lastName(): # TK-004 Проверка полей Имя и Фамилия на корректность ввода на странице регистрации
    ...
    for key in consts.registerKeysDict:
        ...
        for j in range(len(values)):
            ...
            if j < len(values) - 1:
                ...
                time.sleep(1)  # Для наглядности - в реальности тестирования закомментировать
...
def test_register_passwordConfirm(): # TK-007 Проверка корректной работы поля Проверка пароля, на странице Регистрации
    driver, wait = get_driver()
    actionChain = ActionChains(driver)
